...
Setting up signing is discussed in more detail [below](#signing-commits).
...
> **Tips:** 
> 
> 1. If a commit is created that did not include the `-s` or `--signoff` flag, the original commit message can be edited by using the `git commit -s --amend` command. A "force push" must be done afterward to add the amended commit to a PR.
...
To sign a commit, you add either the `--gpg-sign` or `-S` (capital `S`) flag to the commit. You can define an alias to include this flag, like we discussed above for DCO sign**off**, but we'll discuss using a configuration setting that does signing for all commits automatically. Similarly for signoff, if you forget to sign a commit, try using `git commit -S -s --amend ...`
...
Now create a _signing key_ in your [GitHub account](https://github.com/settings/keys) ( again, if you don't already have one). Click the green "New SSH key" button. Make sure you select "Signing key" in the menu for the key type. Name the key and paste the contents of your newly-created (or previously-existing) public key file, `~/.ssh/id_ed25519.pub`.
...
git config --global user.signingkey ~/.ssh/id_ed25519.pub # or whatever /PATH/TO/.SSH/KEY.PUB file
